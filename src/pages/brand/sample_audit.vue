<template>
	<div class="w_100 h_100 flex fc">
		...
		<div class="table_content" id="table_content">
			...
			<CustomTable tableName="sampleAudit" :tableHeight="table_height" :titleList="titleList" :tableData="tableData" :loading="loading" @selectionChange="selectionChange" @auditFn="setFn($event,'only')"/>
		</div>
	...
	export default{
		...
		methods: {
			...
			getData(){
				...
				this.loading = true;
				resource.sendGoodsList(arg).then(res => {
					if(res.data.code == 1){
						this.loading = false;
			...
			auditConfirm(){
				...
				resource.auditSendAudit(arg).then(res => {
					if (res.data.code == 1) {
