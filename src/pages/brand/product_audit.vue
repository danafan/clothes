	...
	<div class="w_100 h_100 flex fc">
		...
		<div v-show="unfold">
			<el-form :inline="true">
				...
				<el-form-item>
					<el-checkbox size="medium" :true-label="2" :false-label="0" v-model="supplier_status">资料通过</el-checkbox>
					<el-checkbox size="medium" :true-label="2" :false-label="0" v-model="send_status">寄样通过</el-checkbox>
					<el-checkbox size="medium" :true-label="2" :false-label="0" v-model="quality_inspection_status">质检通过</el-checkbox>
				</el-form-item>
				<el-form-item>
		...
		<!-- 上传品牌款号 -->
		<custom-dialog dialogTitle="上传品牌款号" ref="uploadBrandKhDialog" @callback="uploadBrandKh">
			<el-form class="dialog_form">
				<el-form-item label="品名：">
					<div class="default_color f14 fw400">{{upload_goods_name}}</div>
				</el-form-item>
				<el-form-item label="品牌款号：" required>
					<el-input style="width:240px" v-model="brand_ksbm"></el-input>
				</el-form-item>
			</el-form>
		</custom-dialog>
		<!-- 审核/批量审核 -->
		<custom-dialog dialogTitle="审核" ref="auditDialog" @callback="auditConfirm">
			<el-form label-position="left">
				...
				<el-form-item label="知核意见：">
					<el-input style="width:256px;" :rows="4" type="textarea" v-model="brand_zhyj" clearable></el-input>
				</el-form-item>
				<el-form-item label="品牌款号：">
					<el-input style="width:256px" v-model="audit_brand_ksbm"></el-input>
				</el-form-item>
			</el-form>
		</custom-dialog>
		<!-- 批量上传品牌款号 -->
		<custom-dialog dialogTitle="上传品牌款号" ref="uploadBrandKhDialog" :showConfirm="false">
			<el-form class="dialog_form">
				<el-form-item>
					<div class="flex">
				...
				<el-form-item>
					<div class="flex">
						...
						<div class="default_color f12 mr24">2.填写品牌款号</div>
	...
	export default{
		data(){
			return{
				...
				brand_ksbm:"",						//输入的品牌款号
				type:1,								//审核默认选中类型
				brand_audit_remark:"",				//审核意见
				brand_zhyj:"",						//知核意见
				audit_brand_ksbm:"",				//品牌款号
		...
		methods: {
			...
			getData(){
				...
				this.loading = true;
				resource.brandGoodsList(arg).then(res => {
					if(res.data.code == 1){
						this.loading = false;
			...
			//点击审核/上传品牌款号
			setFn(arg,ref_name){
				if(ref_name == 'uploadBrandKhDialog'){
					this.upload_goods_name = arg.goods_name;
			...
			uploadBrandKh(){
				if(this.brand_ksbm == ''){
					this.$message.warning('请输入品牌款号！');
					return;
				}
				let arg = {
					goods_id:this.goods_id,
					brand_ksbm:this.brand_ksbm
				}
				resource.addBrandKsbm(arg).then(res => {
					if (res.data.code == 1) {
			...
			importFile(file){
				resource.importBrandKsbm({file:file}).then(res => {
					if (res.data.code == 1) {
						...
						//获取商品审核列表
						this.getData();
						this.$refs.uploadBrandKhDialog.show_dialog = false;
					}else{
						this.$message.warning(res.data.msg);
					}
				})
			...
			auditConfirm(){
				let arg = {
					goods_id:this.goods_id,
					type:this.type,
					brand_zhyj:this.brand_zhyj,
					brand_audit_remark:this.brand_audit_remark,
					brand_ksbm:this.audit_brand_ksbm
				}
				resource.brandGoodsAudit(arg).then(res => {
					if (res.data.code == 1) {
						this.$message.success(res.data.msg);
						//获取商品审核列表
						this.getData();
